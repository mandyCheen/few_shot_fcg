    ...
    for model in models:
        for parm in parms:
            for exp in expList:
                ...
                print("exp: ", exp)
                options["settings"]["name"] = exp+f"_LabelPropagation_alpha0.7_k20_{model}_layers{parm[0]}_128_{parm[1]}_{parm[2]}"
                ...
                try:
                    ...
                    open(f"./logs/Error_lp_models_exp_{date}.txt", "a").write(f"{now}: {exp} {model} seed:{seed} {e}\n")
